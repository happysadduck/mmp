...
def main():
    ...
    if not index_files:
        ...
        manual = input("Please manually enter the complete path of the index file (press Enter to exit):").strip()
        ...
        for i, f in enumerate(index_files, 1):
            print(f"{i}. {f}")
        choice = input("Please enter the number or directly input the file name (for example, 1.12.json):").strip()
        if choice.isdigit():
            idx = int(choice) - 1
